 ...
 * <br>A1：建立一个 K 的最小堆，筛选掉 N - K 个最小元素，堆中留下的就是 K 个最大元素，时 O(Nlog(K)) 空 O(K)
 * <br>A2（最佳）：借鉴快排，当发现正好有 K 个数大于 pivot 时中止。时 O(N)，空 O(N)
 * <br>分布式扩展，每个节点存储一部分数据，通过 rpc 返回大于 pivot 的个数然后聚合再决定下一个 pivot
 *
 * @author lbli
 */
